...
program
  ...
  .action(async () => {
    ...
    if (!packageJson.dependencies || !packageJson.dependencies.react) {
      ...
      console.error(
        chalk.red("error: please install react  before trying again")
      );
      process.exit(1);
    } else {
    ...
    fs.writeFileSync(
      configFileName,
      `export const config = { primaryLanguage: '${answers.primaryLanguage}' };`
    );
    ...
    if (!fs.existsSync(translationFolderPath)) {
      ...
      await delay(delayTime);
      console.log(
        chalk.green(`created translations folder at: ${translationFolderPath}`)
      );
    } else {
      await delay(delayTime);
